...
<body>
	...
	<div class="search_top result_bottom bottom">
		共<b id="totalCount">0</b>条、近月<b id="sumGoods">0</b>单
	</div>
		...
		$(function() {
			...
			function searchData(currentPage) {
				...
				$
						.ajax({
							type : "POST",
							url : '${pageContext.request.contextPath}/weixin/goodsSource/getGoodsSourceInfo_list',
			...
			function fillDataOne(data) {
				...
				if (goodsName!=undefined&&goodsName.length > 3) {
				...
				var name = data.GNAME != null ? data.GNAME : data.GCOMPANYNAME;
				if (name == null) {
					name = "*****";
				} else {
				...
				var headurl;
				var rightLogo = '<a href="javascript:callPhone('
						+ data.GOODSPHONE
						+ ')"><img class="icons" alt="电话" src="${pageContext.request.contextPath}/static/img/call.png"></a>';
				if (data.HEADPHOTOURL != null && data.HEADPHOTOURL != "") {
					headurl = '<div class="hot_play"><ul><li><img class="car_icons goods_icons" alt="货主"	src="${pageContext.request.contextPath}/upload/'
							+ (data.HEADPHOTOURL != null ? data.HEADPHOTOURL
									: "")
							+ '"/><span class="qiang"></span></li></ul></div>';
				} else {
					headurl = '<div class="hot_play"><ul><li><img class="car_icons goods_icons " alt="货主"	src="${pageContext.request.contextPath}/static/images/weixin/ic_user.png"/><span class="qiang"></span></li></ul></div>';
				}
				var quotestatus = data.DEALSTATUS;
				var phonelogo = "";
				if (quotestatus != null && quotestatus != '0') {
					phonelogo = "weixinnotcall";
				} else {
					phonelogo = "weixincall";
				}
				var html = '<div class="data_div" style=" margin-top: 10px;" onclick="dataEdit('
						+ "'"
						+ data.GOODSID
						+ "'"
						+ ')">'
						...
						+ '</div>'
						+ '<div class="dm_middle">'
						+ '<div style="text-align: center; color: #FFBB66"> <span class="dmSpan"> '
						+ goodsName
						+ "</span>"
						+ weight
						+ data.WEIGHTUNIT
						...
						+ '<div class="dm_bottom">'
						+ '截止时间：'
						+ (data.GDSHORTSENDTIME != null ? data.GDSHORTSENDTIME
								: "")
						+ '时 前'
						+ '</div><!-- dm_bottom -->'
						+ '</div><!-- drw_middle end -->'
			...
			window.dataEdit = function(id) {
				if ($("#loginPhone").val() != null
						&& $("#loginPhone").val() != "") {
					/* window.location.href="weixin/goodsSource/goodsSource_detail_hall?GOODSID=" + id; */
					window.location.href = "weixin/goodsSource/goodsSource_detail?GOODSID="
							+ id;
				} else {
					alert("请先绑定或注册新的账号");
				}
