...
<html>
<head>
<title>${fns:getConst('SYSTEMTITLE')}--我的竞价</title>
	...
	<div class="search_top top">
		<div class="search_center">
			<ul class="search_ul contype_per">
				<li class="long_li selected">全部</li>
				<li>竞价中</li>
				<li>已中标</li>
				<li>未中标</li>
			</ul>
		...
		$(function() {
			...
			function searchData(currentPage) {
				...
				$
						.ajax({
							type : "POST",
							url : '${wx}/weixin/bid/getbiddingList',
							data : {
								biddingstatus : searchConditions.biddingstatus,
								pageNo : searchConditions.pageNo,
								pageSize : searchConditions.pageSize,
								"bidinfo.goods.shipAddress.province" : searchConditions.fromprovince,
								"bidinfo.goods.shipAddress.city" : searchConditions.fromcity,
								"bidinfo.goods.consignAddress.province" : searchConditions.toprovince,
								"bidinfo.goods.consignAddress.city" : searchConditions.tocity
			...
			function fillDataOne(data) {
				var price = "";
				var weight = "";
				if (data.bidinfo.expectedprice * 1 >= 100000) {
					var p = data.bidinfo.expectedprice / 10000;
					p = p.toString();
					...
					price = data.bidinfo.expectedprice + "元";
				}
				...
				var goodsName = data.bidinfo.goods.goodsname;
				...
				if (data.bidinfo.goods.shipAddress != undefined
						&& data.bidinfo.goods.shipAddress != null
						&& data.bidinfo.goods.shipAddress != "") {
					fromAddress = (data.bidinfo.goods.shipAddress.province + data.bidinfo.goods.shipAddress.city)
							.substring(0, 6);
				}
				if (data.bidinfo.goods.consignAddress != undefined
						&& data.bidinfo.goods.consignAddress != null
						&& data.bidinfo.goods.consignAddress != "") {
					toAddress = (data.bidinfo.goods.consignAddress.province + data.bidinfo.goods.consignAddress.city)
							.substring(0, 6);
				}
				var bidlogo = '<div  style="width:50%;height:30px;line-height:33px;-moz-border-radius:6px;-webkit-border-radius:6px;border-radius:6px; margin:0 auto;padding-bottom:0px;background-color:#AAAAAA;text-align: center;"><span style="font-weight: bold;color:white;">未中标</span></div>';
				if (data.biddingstatus == "中标") {
					bidlogo = '<div  style="width:50%;height:30px;line-height:33px;-moz-border-radius:6px;-webkit-border-radius:6px;border-radius:6px; margin:0 auto;padding-bottom:0px;background-color:rgba(247, 26, 26, 0.85); text-align: center;"><span style="font-weight: bold;color:white;">中标</span></div>';
				}
				...
				var html = '<div class="data_div"onclick="dataDetails(' + "'"
						...
						+ ((data.biddingcode).substring(0, 18) + "...")
						+ '</div><div class="frt" style="float:right;marign:0;color:#AAAAAA;">截标:'
						+ ((data.bidinfo.endtime).substring(0, 10))
						+ '</div></div>' + '<div class="data_row">' + ''
						+ '<div class="top_text flt"style="text-indent:5px;">'
						+ fromAddress + ' - ' + toAddress + '</div>'

						+ '<div style="width:28%;height:30%;float:left;">'
						+ bidlogo + '</div>' + '	</div>'
						+ '<div class="data_row data_btm">'
						...
						+ '</div><div style="color:#AAAAAA;">' + '拦标价:' + price
						+ '</div>';
				if (data.biddingstatus == '2') {
					html = html
							+ '<div class="ic_img" style="margin:0px;"></div>';
				}
				html = html + '</div></div>';
			...
			window.dataDetails = function(id) {
				window.location.href = "${wx}/weixin/bid/toBiddingDetails?id="
						+ id;
			};
