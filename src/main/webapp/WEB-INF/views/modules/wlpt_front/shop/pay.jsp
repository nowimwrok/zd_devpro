...
<div class="shop_content">
    ...
    <table class="pur_tab" style="margin-bottom: 30px;    border-bottom: 1px dashed #ccc;">
        ...
        <tr>
            ...
            <td class="tab_right" style="width: 50%;">${not empty shop.total?shop.total:'0.00'}<c:if
                    test="${shop.shotName=='zd_coin'}">元</c:if></td>
        </tr>
        ...
        <c:if test="${shop.shotName=='you_qi'}">
            <tr>
                <td class="tab_left">
                    账户卓币余额：
                </td>
                <td class="tab_right"><span
                        id="availablecoin">${not empty user.userPurse.availablecoin?user.userPurse.availablecoin:'0.00'}</span>币
                </td>
            </tr>

        </c:if>
        <tr>
            <td class="tab_left">支付方式：</td>
            <td class="tab_right">
                <c:if test="${shop.shotName=='zd_coin'}">
        ...
        <tr style="height: 70px;">
            ...
            <td class="tab_right" style="width: 50%;"><input id="password" type="password"/></td>
        </tr>
        <tr class="colspan2">
            <td colspan="2" class="btn_td" style="padding-bottom: 30px;text-align:center;">
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.value == 6) {
                                    ...
                                    $(".loading").hide();
                                    $.MsgBox.Alert("支付提示", "购买失败，请查看是否已绑定一卡通账号");
                                    $("#mone").show();
                                    $("#sub").hide();
                                    ...
                                    $(".loading").hide();
                                    $.MsgBox.Alert("支付提示", "应用正在日结中，等银行结算完毕再开放使用，结算时间为1-2小时");
                                    $("#mone").show();
                                    $("#sub").hide();
                                } else {
                                    $(".loading").hide();
