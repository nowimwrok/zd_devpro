...
<ul class="qiye_menu">
		...
		<li data-type="3" class="qiye_menu_1"><a href="${wlpt}/wlpt/consume/toTransferPetro" target="content" style="color: #000;">油气转账</a></li>
...
<div class="withdraw_cash_wrap">
    <p class="user_data">
        <img src="/static/images/wlptfront/line-left.png"><span class="title_blue">油气</span><span class="title_orange">转账</span><img
    ...
    <table>
        <tr>
            ...
            <td class="right_con right_con_1" style="width:25%;font-size: 16px;">
                <input type="text" id="searchinfo" placeholder="用户名/手机号/一卡通卡号">
            </td>
            <td style="width:25%;">
        ...
        <tr>
            <td class="left_title">转账油气：</td>
            <td class="right_con" colspan=3><input type="text" id="num" style="height: 28px;line-height: 28px;">&nbsp;元&nbsp;&nbsp;&nbsp;<span
                    class="count_state">可用油气：</span><span
                    class="count_green">${not empty user.userPurse.petrolbalance ? user.userPurse.petrolbalance:'0.00'}</span>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="right_con please_note" colspan=3>请注意：向账户转账，油气从账户油气扣减，转账油气大于0并且小于账户油气，如有问题，请致电卓大客服办理</td>
        </tr>
        <tr>
            <td class="left_title"></td>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == false) {
                            ...
                            $.ajax({
                                url: "${wlpt}/wlpt/consume/transferPetro",
                                dataType: 'json',
                                ...
                                success: function (data) {
                                    if (data.value == 1) {
                                        ...
                                        $.MsgBox.Alert("转账提示", "账户油气不足");
                                        $(".loading_lock").hide();
                                    } else if (data.value == 2) {
                                        $.MsgBox.Alert("转账提示", "用户不存在");
                                        $(".loading_lock").hide();
                                    }else if (data.value == 13) {
                                        $.MsgBox.Alert("转账提示", "转账双方必须先绑定一卡通账号");
                                        $(".loading_lock").hide();
                                    } 
                                    else {
