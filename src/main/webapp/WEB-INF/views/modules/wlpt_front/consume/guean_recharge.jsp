...
<ul class="qiye_menu">
    ...
    <li data-type="2" ><a href="${wlpt}/wlpt/consume/toTransferCoin" target="content" style="color: #000;">卓币转帐</a></li>
    ...
    <li data-type="4" class="qiye_menu_1">保证金充值</li>
</ul>
        <div class="withdraw_cash_wrap">
        <p class="user_data">
            <img src="/static/images/wlptfront/line-left.png"><span class="title_blue">保证金</span><span class="title_orange">充值</span><img src="/static/images/wlptfront/line-right.png">
        </p>
            <table>
                <tr>
                    ...
                    <td class="right_con right_con_1" style="width:25%;font-size: 16px;">
		                ...
		                <span id="guaranteemone">0.00</span>
                        </c:when>
                        <c:otherwise>
                ...
                <tr>
                    <td class="left_title" style="width:20%;">当前保证金：</td>
                    <td class="right_con right_con_1" style="width:25%;font-size: 16px;">
                        ${user.userPurse.guaranteemone }&nbsp;元
                    </td>
                    <td style="width:25%;"></td>
                    <td style="width:25%;"></td>
                </tr>
                <tr>
                    <td class="left_title">充值保证金：</td>
                    <td class="right_con" colspan=3><input type="number" min="1" id="num" style="height: 28px;line-height: 28px;">&nbsp;&nbsp;元</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="right_con please_note" colspan=3>请注意：仅支持余额充值，在线支付成功后。充值金额会在一分钟内到账；如有疑问，请致电卓大物流客服办理</td>
                </tr>
                <tr>
                    <td class="left_title"></td>
	           ...
	           if(data.status==200){
	        	   ...
	        	   for(i=0;i<slaveAccounts.length;i++){
	        		   if(slaveAccounts[i].accountType==4){
	        			   $("#guaranteemone").html(slaveAccounts[i].availableBalance);
	        		   }
	        	   }
...
$("#btnSubmit").click(function () {
    ...
    if (nul) {
    	...
    	var total="${not empty user.userPurse.availablebalance ? user.userPurse.availablebalance:'0.00'}"
    	//var total = $("#guaranteemone").html();
    	if(num*1>total*1){
    		$("#num").tips({
                side: 3,
                msg: '可用余额不足',
    		...
    		$.ajax({
	            ...
	            success: function (data) {
	                if (data == false) {
	           		 ...
	           		 $.ajax({
	     	            url: "${wlpt}/wlpt/consume/gueanRecharge",
	     	            dataType: 'json',
	     	            data:{"consumemony":num},
	     	            type: "post",
	     	            success: function (data) {
	     	                if (data.value == 1) {
	     	                	...
	     	                	$.MsgBox.Alert("充值提示", "用户信息缺失,充值失败");
		     	                $(".loading_lock").hide();
	     	                }else if(data.value==-1){
	     	                	$.MsgBox.Alert("充值提示", "账户系统异常,充值失败");
		     	                $(".loading_lock").hide();
	     	                }
	     	                else {
