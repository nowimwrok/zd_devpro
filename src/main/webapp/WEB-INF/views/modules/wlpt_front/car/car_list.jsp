...
<p class="user_data">
    <img src="${ctxStatic }/images/wlptfront/line-left.png"><span class="title_blue">车辆</span><span
...
<div class="select_title">
    <form action="" method="get" class="search-bar" style="text-align: center">
        ...
        <input id="carlength" type="text" ng-model="searchData.carlength" class="car_search car_phone_search"
               placeholder="车长"/>
        <select id="issocialcar" ng-model="searchData.issocialcar" class="car_search car_phone_search" placeholder="归属"">
        <option value=''></option>
...
<table class="goods-manage">
    ...
    <tr ng-repeat="x in data">
        ...
        <td>
            <p>{{x.issocialcar}}|{{x.status}}</p>
            <p>{{x.cartype}}|车长{{x.carlength}}米|载重{{x.loadweight}}吨</p>
            <p>共成交{{x.tradecount}}笔交易</p>
        </td>
        <td>
            <p>当前位置：{{x.gpsloc.info!=null?x.gpsloc.info:"暂无定位信息"}}</p>
            <p>定位时间：{{x.gpsloc.gpstime!=null?x.gpsloc.formatgpstime:"暂无定位信息"}}</p>
        </td>
        <td>
            <p class="last"><a ng-if="x.status=='未认证'" href="${wlpt}/wlpt/car/toSave?id={{x.id}}" target="content"
                               class="btn btn-warning">编辑</a></p>
            <p class="last"><a ng-if="x.status=='未认证'" href="${wlpt}/wlpt/car/getCarInfo?id={{x.id}}" target="content"
                               class="btn btn-warning">申请认证</a></p> 
            <p class="last"><a ng-if="x.status=='已申请'||x.status=='已认证'" href="${wlpt}/wlpt/car/getCarInfo?id={{x.id}}" target="content"
                               class="btn btn-warning">查看</a></p> 
            <p class="last"><a ng-if="x.status=='已认证'" href="${wlpt}/wlpt/carsource/toSave?type=0&carid={{x.id}} " target="content"
                               class="btn btn-warning">发布车源</a></p>    
