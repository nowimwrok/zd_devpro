<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ include file="/WEB-INF/views/include/taglib.jsp" %>

<!DOCTYPE html>
<html lang="en">
<head>
    <link rel="stylesheet" href="/static/modules/wlpt/front/css/analysis/global.css"/>
    <style type="text/css">
        html, body {
            margin: 0;
            height: 100%;
        }

        #carTypediv, #monthdiv {
            float: left;
            height: 450px;
        }

        .rightdiv {
            width: 240px;
            height: 465px;
            position: fixed;
            bottom: 100px;
            right: 3%;
            background-color: transparent;
            z-index: 22;
            border: solid 1px #FFFFFF;
        }

        .rtitle {
            height: 45px;
            width: 100%;
            font-size: 16px;
            color: #f1f2ed;
            text-align: center;
            line-height: 45px;
        }

        .search-box {
            width: 100%;
            background-color: #2d3641;
        }

        .input-box {
            width: 82%;
            outline: 0;
            float: left;
        }

        .form-control {
            display: inline-block;
            height: 34px;
            padding: 6px 10px;
            width: 100%;
            line-height: 1.42857143;
            color: #fff;
            background-color: #2d3641;
            background-image: none;
            border: 0;
            -webkit-box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075);
            box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075);
            -webkit-transition: border-color ease-in-out .15s, -webkit-box-shadow ease-in-out .15s;
            -o-transition: border-color ease-in-out .15s, box-shadow ease-in-out .15s;
            transition: border-color ease-in-out .15s, box-shadow ease-in-out .15s;
        }

        .search-btn {
            background-color: #2d3641;
            padding: 5px 0;
            display: inline-block;
            vertical-align: top;
            line-height: 2.2rem;
            height: 34px;
            width: 15%;
            cursor: pointer;
        }

        .input-group-addon {
            display: block;
            border-left: 1px solid #074376;
            height: 24px;
            margin-top: 2px;
        }

        a {
            background: transparent;
            text-decoration: none;
            color: #0088cc;
        }

        .input-group-addon .i {
            display: inline-block;
            background: url('http://bcscdn.baidu.com/baidu-qianxi/chuxing/images/icon-common.png');
            vertical-align: middle;
            cursor: pointer;
            width: 23px;
            height: 23px;
            background-position: -59px -170px;
            vertical-align: top;
        }

        .i {
            display: inline-block;
            background: url('http://bcscdn.baidu.com/baidu-qianxi/chuxing/images/icon-common.png');
            vertical-align: middle;
            cursor: pointer;
        }

        .div_list_container .row {
            margin-bottom: 18px;
        }

        .serial-number {
            border-radius: 4px;
            height: 20px;
            width: 20px;
            line-height: 20px;
            background-color: #008cff;
            text-align: center;
            display: inline-block;
            margin-left: 10px;
            margin-right: 5px;
        }

        .i12 {
            width: 45px;
            height: 15px;
            background-position: -48px -137px;
        }

        .city-name {
            width: 50px;
            display: inline-block;
            color: #fff;
        }

        a {
            background: transparent;
            text-decoration: none;
            color: #0088cc;
        }

        .col {
            float: left;
        }

        .outer-wrap {
            height: 370px;

        }

        /********/
        .business {
            width: 140px;
            height: 345px;
            position: fixed;
            bottom: 100px;
            left: 3%;
            background-color: transparent;
            z-index: 22;
            border: solid 1px #FFFFFF;
        }

        .busin_box {
            width: 100%;
            height: 255px;
            background-color: transparent;
            display: -moz-box;
            display: -webkit-box;
            -webkit-align-content: center; /* 默认横向划分  vertical 为纵向划分*/
            -moz-box-orient: vertical;
            -webkit-box-orient: vertical
        }

        .busin_box > div {
            width: 80%;
            height: 20%;
            border: 1px solid saddlebrown;
            -webkit-box-flex: 1;
            -moz-box-flex: 1;
            margin: 0px auto;
            margin-bottom: 20px;

        }

        .busin_block dl dt, .busin_block dl dd {
            float: left;
        }

        .busin_block dl dt {
            width: 30%;
            float: left;
            height: 32px;
            text-align: center;
            line-height: 32px;
            background-color: rgb(249, 249, 249);
            border: 2px solid red;
            border-radius: 48%;
            margin: -8px 0px 0px 5px;
            font-weight: bolder;
        }
        .busin_block dl dd {
            font-size: 14px;
            color: #FFFFFF;
        }

        input {
            outline: none;
        }
    </style>
    <script src="${ctxStatic }/jquery/jquery-1.8.3.min.js" type="text/javascript"></script>
    <script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=YYnbCRSKd9CGoqfP6PcM5wNo"></script>
</head>
<body>

<div style="width: 50%;height: 50%;display: inline;">
    <div id="flowechart" class="echart-content" style="display: block;height: 100%;width:100%;"></div>
</div>
<div class="business">
    <div class="rtitle">今日业务</div>
    <div class="busin_box">
        <div class="busin_block">
            <dl>
                <dt>招</dt>
                <dd style="width: 45%;float: left;-webkit-margin-start: 15px;"><span id="BIDWEIGHT">32902</span>吨</dd>
            </dl>
        </div>
        <div class="busin_block">
            <dl>
                <dt>运</dt>
                <dd style="width: 45%;float: left;-webkit-margin-start: 15px;"><span id="TRADEWEIGHT">32902</span>吨</dd>
            </dl>
        </div>
        <div class="busin_block">
            <dl>
                <dt>达</dt>
                <dd  style="width: 45%;float: left;-webkit-margin-start: 15px;"><span id="ARRIVEWEIGHT">32902</span>吨</dd>
            </dl>
        </div>
        <div style="margin: 0px auto" class="busin_block">
            <dl>
                <dt>结</dt>
                <dd  style="width: 45%;float: left;-webkit-margin-start: 15px;"><span id="SETTLEMONEY">32902</span>元</dd>
            </dl>
        </div>
    </div>
    <div class="rtitle">
                <span class="input-icon">
                <input id="nowDate"
                       style="border:none;text-align: center;background-color: transparent;width: 80%;height: 40px;font-size: 16px;color: #FFFFFF;"
                       onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value=""/>
                    <i id="nav-search-icon" class="icon-calendar" style="line-height: 45px;margin-left: 10px;"></i>

                </span>
    </div>
</div>
<div id="tradeline" class="rightdiv" style="display: none;">
    <div class="rtitle">运价指数</div>
    <div class="search-box" style="width: 100%;display: none;">
        <input type="text" id="input_cityName"
               style="width: 75%;margin-top:5px;border:none;background-color: transparent;" placeholder="请输入省/市"
               class="form-control input-box"
               value="邯郸市">
        <div class="search-btn" id="btn_city_search"
             style="width: 15%;padding: 0px;margin: 0px;margin-top:5px;">
            <a href="javascript:;" class="input-group-addon ">
                <i class="i i02" style="margin-left: 12px;"></i>
            </a>
        </div>
    </div>
    <div class="outer-wrap" style="margin-top: 13px;overflow: hidden;">
        <div class="div_list_container" id="city_list" style="position:relative;top:0">
            <div class="row">
                <span class="serial-number col" data="index">1</span>
                <a href="javascript:;" class="col city-name" data="startName" title="上海">上海</a>
                <i class="col i i12"></i>
                <a href="javascript:;" class="city-name" data="endName" title="北京">北京</a>
            </div>
        </div>
    </div>
</div>
<div id="allmap" style="display:none;"></div>

<script type="text/javascript" src="/static/echarts/echarts.min.js"></script>
<script type="text/javascript" src="/static/echarts/echarts-all.js"></script>
<script type="text/javascript" src="${ctxStatic}/modules/wlpt/front/js/jquery.pcc.js"></script>
<script src="${ctxStatic }/modules/wlpt/front/js/My97DatePicker/WdatePicker.js" type="text/javascript"></script>

<script type="text/javascript">
    // 百度地图API功能通过IP获取当前城市
    var map = new BMap.Map("allmap");
    var point = new BMap.Point(116.331398, 39.897445);
    map.centerAndZoom(point, 12);

    function myFun(result) {
        var cityName = result.name;
        map.setCenter(cityName);
        $("#input_cityName").val(cityName);
    }
    var myCity = new BMap.LocalCity();
    myCity.get(myFun);
</script>

<script type="text/javascript">
    $(function () {
        var date = new Date();
        var month = date.getMonth() + 1;
        var day = date.getDate();
        if (month < 10) {
            month = "0" + month;
        }
        if (day < 10) {
            day = "0" + day;
        }
        $("#nowDate").val(date.getFullYear() + "-" + month + "-" + day);
        $("#nowDate").blur(function () {
            getBuinessData();
        });
        getBuinessData();
        ///加载每日业务数据
        function getBuinessData() {
            $.ajax({
                type: "POST",
                url: '${wlpt}/wlpt/dataCenter/getBusinessData',
                dataType: 'json',
                data: {searchTime: $("#nowDate").val()},
                cache: false,
                success: function (data) {
                    if (data.data.varList.length > 0) {
                        var nowdata = data.data.varList[data.data.varList.length - 1];//当数据对象
                        $("#BIDWEIGHT").html(nowdata.bidweight*1==0?"4.5千":loaddecml(nowdata.bidweight*1));
                        $("#TRADEWEIGHT").html(nowdata.tradeweight*1==0?"56.5万":loaddecml(nowdata.tradeweight*1));
                        $("#ARRIVEWEIGHT").html(nowdata.arriveweight*1==0?"190万":loaddecml(nowdata.arriveweight*1));
                        $("#SETTLEMONEY").html(nowdata.arrivemoney*1==0?"1265万":loaddecml(nowdata.arrivemoney*1));
                    }else{
                    	$("#BIDWEIGHT").html("4.5千");
                        $("#TRADEWEIGHT").html("56.5万");
                        $("#ARRIVEWEIGHT").html("190万");
                        $("#SETTLEMONEY").html("1265万");
                    }
                }
            });
        }

        /**
         * 优化数据分析大数据
         * @param val
         * @returns {*}
         */
        function loaddecml(val) {
            //值
            var p = 0;
            ///单位
            var vol = "";
            if (val >= 1000 && val < 10000) {
                p = val / 1000;
                vol = "千";
            } else if (val >= 10000 && val < 1000000) {
                p = val / 10000;
                vol = "万";
            } else if (val >= 1000000 && val <10000000) {
                p = val / 1000000;
                vol = "百万";
            } else {
                p = val;
            }
            p = p.toString();
            var index = p.indexOf(".", 0);
            index = index >= 0 ? index : p.length;
            index += 2;
            index = index < p.length ? index : p.length;
            val = p.substring(0, index) + vol;
            return val;
        }

        $("#btn_city_search").click(function () {
            loadtrateLine();
        });
        loadtrateLine();
        function loadtrateLine() {
            $.ajax({
                type: "POST",
                url: '${wlpt}/wlpt/dataCenter/getTrateLineData',
                dataType: 'json',
                data: {startAddress: $("#input_cityName").val()},
                cache: false,
                success: function (data) {
                    var fillHtml = "";
                    $("#city_list").html("");
                    for (var i = 0; i < data.data.varList.length; i++) {
                        var nowdata = data.data.varList[i];//当数据对象
                        fillHtml += loadData(nowdata, i + 1);
                    }
                    $("#city_list").html(fillHtml);
                    listScroll('city_list');
                }
            });
        }

        function loadData(data, i) {
            var start = data.startaddress.replace("市", "");
            var end = data.endaddress.replace("市", "");
            var valpr = end + data.priceinfo.newprice + '元/吨'
            var html = '<div class="row" style="padding: 0px;margin: 0px 0px 18px 0px;">'
                    + '<span class="serial-number col" data="index">' + i + '</span>'
                    + '<a href="javascript:;" class="col city-name"  data="startName" title="' + start + '">' + start + '</a>'
                    + '<i class="col i i12"></i>'
                    + '<a href="javascript:;" class="city-name" data="endName" style="width: 110px;" title="' + end + '">' + valpr + '</a>'
                    + '</div>';
            return html;
        }

        //列表滚动方法
        function listScroll(domid) {//传入滚动table的id
            var $domid = $("#" + domid + "");
            $domid.css("top", 0);
            var inittop = $domid.offset().top;//初始位置top
            var trheight = $domid.find(".row").height() + 18;//行高
            function scrollRun() {
                if (inittop - $domid.offset().top >= $domid.height() - $domid.parent("div").height()) {
                    //  $domid.offset({top:inittop})
                    $domid.css("top", 0);
                } else {
                    // var dd=$domid.css("top").replace("px","")-trheight;
                    //$domid.offset({top:$domid.offset().top-trheight});
                    $domid.animate({top: $domid.css("top").replace("px", "") - trheight}, 1500)
                }
            };
            if ($domid.find(".row").length > 7) {
                var myInter = setInterval(scrollRun, 2500);
                $domid.mouseenter(function () {
                    clearInterval(myInter);
                });
                $domid.mouseleave(function () {
                    myInter = setInterval(scrollRun, 2500);
                });
                $domid.mouseenter();
                $domid.mouseleave();
            }
        };

        /* $("#input_cityName").PCC({
            hasCounty: false,
            width: 400,
            height: 320,
            url: "${wlpt}/wlpt/chinaarea/getArea",
            //closeIcon: "static/images/pic12.gif",
            complete: function (data) {
                var str = "";
                if (data.city) {
                    if (data.city.NAME != data.province.NAME) {
                        str += data.city.NAME;
                    } else {
                        str += data.province.NAME;
                    }
                }
                if (str != " ") {
                    $("#input_cityName").val(str);
                } else {
                    $("#input_cityName").val("");
                }
            }
        }); */
    });
</script>
<script type="text/javascript">
    $(function () {

        var dom = document.getElementById("flowechart");
        var myChart = echarts.init(dom);
       // var cityPoint = $.myConfig.cityPoint;
        var dataVar = [];///整体线路数组
        var dataall = [];///所有线路
        var endVar = [];//终点数据
        var fromcity = "";
        getDataCount();
        $("#btn_city_search").click(function () {
            getDataCount();
        });
        function getDataCount() {
            fromcity = $('#input_cityName').val();
            dataVar = [];///整体线路数组
            dataall = [];///所有线路
            endVar = [];//终点数据
            myChart.setOption({});
            $.ajax({
                type: "POST",
                url: '${wlpt}/wlpt/dataCenter/getTradeFlowData',
                dataType: 'json',
                data: {fromcity: fromcity},
                cache: false,
                success: function (data) {
                    for (var i = 0; i < data.data.varList.length; i++) {
                        var nowdata = data.data.varList[i];//当数据对象
                        var STARTADDRESS = nowdata.startaddress.replace("市", "");
                        var ENDADDRESS = nowdata.endaddress.replace("市", "");
                        
                        dataall[i] = [{name: STARTADDRESS}, {name: ENDADDRESS}];
                        dataVar[i] = [{name: STARTADDRESS}, {name: ENDADDRESS, value: nowdata.everycount}];
                        endVar[i] = [{name: ENDADDRESS, value: nowdata.everycount}];
                    }

                    option = {
                        backgroundColor: '#1b1b1b',
                        color: ['gold', 'aqua', 'lime'],
                        tooltip: {
                            trigger: 'item',
                            formatter: '{b}'
                        },
                        dataRange: {
                            min: 0,
                            max: 100,
                            calculable: true,
                            color: ['#ff3333', 'orange', 'yellow', 'lime', 'aqua'],
                            textStyle: {
                                color: '#fff'
                            }
                        },
                        series: [
                            {
                                name: '全国',
                                type: 'map',
                                roam: true,
                                hoverable: false,
                                mapType: 'china',
                                itemStyle: {
                                    normal: {
                                        borderColor: 'rgba(100,149,237,1)',
                                        borderWidth: 0.5,
                                        areaStyle: {
                                            color: '#1b1b1b'
                                        }
                                    }
                                },
                                data: [],
                                markLine: {
                                    smooth: true,
                                    effect: {
                                        show: true,
                                        scaleSize: 1,
                                        period: 30,
                                        color: '#fff',
                                        shadowBlur: 10
                                    },
                                    itemStyle: {
                                        normal: {
                                            borderWidth: 1,
                                            lineStyle: {
                                                type: 'solid',
                                                shadowBlur: 10
                                            }
                                        }
                                    },
                                    data: dataall,
                                },
                                geoCoord: {
                                    '北京': [116.395645, 39.929986],
                                    '上海': [121.487899, 31.249162],
                                    '天津': [117.210813, 39.14393],
                                    '重庆': [106.530635, 29.544606],
                                    //安徽省
                                    '安徽': [117.216005, 31.859252],
                                    '合肥': [117.282699, 31.866942],
                                    '安庆': [117.058739, 30.537898],
                                    '蚌埠': [117.35708, 32.929499],
                                    '亳州': [115.787928, 33.871211],
                                    '巢湖': [117.88049, 31.608733],
                                    '池州': [117.494477, 30.660019],
                                    '滁州': [118.32457, 32.317351],
                                    '阜阳': [115.820932, 32.901211],
                                    '淮北': [116.791447, 33.960023],
                                    '淮南': [117.018639, 32.642812],
                                    '黄山': [118.29357, 29.734435],
                                    '六安': [116.505253, 31.755558],
                                    '马鞍': [118.515882, 31.68852],
                                    '宿州': [116.988692, 33.636772],
                                    '铜陵': [117.819429, 30.94093],
                                    '芜湖': [118.384108, 31.36602],
                                    '宣城': [118.752096, 30.951642],
///福建
                                    '福建': [117.984943, 26.050118],
                                    '福州': [119.330221, 26.047125],
                                    '龙岩': [117.017997, 25.078685],
                                    '南平': [118.181883, 26.643626],
                                    '宁德': [119.542082, 26.656527],
                                    '莆田': [119.077731, 25.44845],
                                    '泉州': [118.600362, 24.901652],
                                    '三明': [117.642194, 26.270835],
                                    '厦门': [118.103886, 24.489231],
                                    '漳州': [117.676205, 24.517065],
                                    ///甘肃102.457625,38.103267
                                    '甘肃': [102.457625, 38.103267],
                                    '兰州': [103.823305, 36.064226],
                                    '白银': [104.171241, 36.546682],
                                    '定西': [104.626638, 35.586056],

                                    '甘南藏族自治州': [102.917442, 34.992211],
                                    '嘉峪关': [98.281635, 39.802397],
                                    '金昌': [102.208126, 38.516072],
                                    '酒泉': [98.508415, 39.741474],

                                    '临夏回族自治州': [103.215249, 35.598514],
                                    '陇南': [104.934573, 33.39448],
                                    '平凉': [106.688911, 35.55011],
                                    '庆阳': [107.644227, 35.726801],
                                    '天水': [105.736932, 34.584319],
                                    '武威': [102.640147, 37.933172],
                                    '张掖': [100.459892, 38.93932],
                                    //广东
                                    '广东': [113.394818, 23.408004],
                                    '广州': [113.30765, 23.120049],
                                    '潮州': [116.630076, 23.661812],
                                    '东莞': [113.763434, 23.043024],
                                    '佛山': [113.134026, 23.035095],
                                    '河源': [114.713721, 23.757251],
                                    '惠州': [114.410658, 23.11354],
                                    '江门': [113.078125, 22.575117],
                                    '揭阳': [116.379501, 23.547999],
                                    '茂名': [110.931245, 21.668226],
                                    '梅州': [116.126403, 24.304571],
                                    '清远': [113.040773, 23.698469],
                                    '汕头': [116.72865, 23.383908],
                                    '汕尾': [115.372924, 22.778731],
                                    '韶关': [113.594461, 24.80296],
                                    '深圳': [114.025974, 22.546054],
                                    '阳江': [111.97701, 21.871517],
                                    '云浮': [112.050946, 22.937976],
                                    '湛江': [110.365067, 21.257463],
                                    '肇庆': [112.479653, 23.078663],
                                    '中山': [113.42206, 22.545178],
                                    '珠海': [113.562447, 22.256915],

                                    //广西
                                    '广西': [108.924274, 23.552255],
                                    '南宁': [108.297234, 22.806493],
                                    '百色': [106.631821, 23.901512],
                                    '北海': [109.122628, 21.472718],
                                    '崇左': [107.357322, 22.415455],
                                    '防城港': [108.351791, 21.617398],
                                    '桂林': [110.26092, 25.262901],
                                    '贵港': [109.613708, 23.103373],
                                    '河池': [108.069948, 24.699521],
                                    '贺州': [111.552594, 24.411054],
                                    '来宾': [109.231817, 23.741166],
                                    '柳州': [109.422402, 24.329053],
                                    '钦州': [108.638798, 21.97335],
                                    '梧州': [111.305472, 23.485395],
                                    '玉林': [110.151676, 22.643974],
//贵州省

                                    '贵州': [106.734996, 26.902826],
                                    '贵阳': [106.709177, 26.629907],
                                    '安顺': [105.92827, 26.228595],
                                    '毕节': [116.395645, 39.929986],
                                    '六盘水': [104.852087, 26.591866],
                                    '铜仁': [109.196161, 27.726271],
                                    '遵义': [106.93126, 27.699961],
                                    '黔西南布依族苗族自治州"': [104.900558, 25.095148],
                                    '黔东南苗族侗族自治州': [107.985353, 26.583992],
                                    '黔南布依族苗族自治州': [107.523205, 26.264536],
                                    //海南省
                                    '海南': [109.733755, 19.180501],
                                    '海口': [110.330802, 20.022071],
                                    '白沙': [109.358586, 19.216056],
                                    '保亭': [109.656113, 18.597592],
                                    '昌江': [109.0113, 19.222483],
                                    '儋州': [109.413973, 19.571153],
                                    '澄迈': [109.996736, 19.693135],
                                    '东方': [108.85101, 18.998161],
                                    '定安': [110.32009, 19.490991],
                                    '琼海': [110.414359, 19.21483],
                                    '琼中': [109.861849, 19.039771],
                                    '乐东': [109.062698, 18.658614],
                                    '临高': [109.724101, 19.805922],
                                    '陵水': [109.948661, 18.575985],
                                    '三亚': [109.522771, 18.257776],
                                    '屯昌': [110.063364, 19.347749],
                                    '万宁': [110.292505, 18.839886],
                                    '文昌': [110.780909, 19.750947],
                                    '五指山': [109.51775, 18.8313066],
//河北省
                                    '河北': [115.661434, 38.61384],
                                    '石家庄': [114.522082, 38.048958],
                                    '保定': [115.49481, 38.886565],
                                    '沧州': [116.863806, 38.297615],
                                    '承德': [117.933822, 40.992521],
                                    '邯郸': [114.482694, 36.609308],
                                    '衡水': [115.686229, 37.746929],
                                    '廊坊': [116.703602, 39.518611],
                                    '秦皇岛': [119.604368, 39.945462],
                                    '唐山': [118.183451, 39.650531],
                                    '邢台': [114.520487, 37.069531],
                                    '张家口': [114.893782, 40.811188],
                                    //河南省
                                    '河南': [113.486804, 34.157184],
                                    '郑州': [113.649644, 34.75661],
                                    '安阳': [114.351807, 36.110267],
                                    '鹤壁': [114.29777, 35.755426],
                                    '焦作': [113.211836, 35.234608],
                                    '开封': [114.351642, 34.801854],
                                    '洛阳': [112.447525, 34.657368],
                                    '漯河': [114.046061, 33.576279],
                                    '南阳': [112.542842, 33.01142],
                                    '平顶山': [113.300849, 33.745301],
                                    '濮阳': [115.026627, 35.753298],
                                    '三门峡': [111.181262, 34.78332],
                                    '商丘': [115.641886, 34.438589],
                                    '新乡': [113.91269, 35.307258],
                                    '信阳': [114.085491, 32.128582],
                                    '许昌': [113.835312, 34.02674],
                                    '周口': [114.654102, 33.623741],
                                    '驻马店': [114.049154, 32.983158],
                                    //黑龙江
                                    '黑龙江': [128.047414, 47.356592],
                                    '哈尔滨': [126.657717, 45.773225],
                                    '大庆': [125.02184, 46.596709],
                                    '大兴安岭': [124.196104, 51.991789],
                                    '鹤岗': [130.292472, 47.338666],
                                    '黑河': [127.50083, 50.25069],
                                    '鸡西': [130.941767, 45.32154],
                                    '佳木斯': [130.284735, 46.81378],
                                    '牡丹江': [129.608035, 44.588521],
                                    '七台河': [131.019048, 45.775005],
                                    '齐齐哈尔': [123.987289, 47.3477],
                                    '双鸭山': [131.171402, 46.655102],
                                    '绥化': [126.989095, 46.646064],
                                    '伊春': [128.910766, 47.734685],
                                    //湖北省
                                    '湖北': [112.410562, 31.209316],
                                    '武汉': [114.3162, 30.581084],
                                    '鄂州': [114.895594, 30.384439],
                                    '恩施': [109.517433, 30.308978],
                                    '黄冈': [114.906618, 30.446109],
                                    '黄石': [115.050683, 30.216127],
                                    '荆门': [112.21733, 31.042611],
                                    '荆州': [112.241866, 30.332591],
                                    '潜江': [112.768768, 30.343116],
                                    '神农架': [110.487231, 31.595768],
                                    '十堰': [110.801229, 32.636994],
                                    '随州': [113.379358, 31.717858],
                                    '天门': [113.12623, 30.649047],
                                    '仙桃': [113.387448, 30.293966],
                                    '咸宁': [114.300061, 29.880657],
                                    '襄樊': [112.176326, 32.094934],
                                    '孝感': [113.935734, 30.927955],
                                    '宜昌': [111.310981, 30.732758],
                                    //湖南省
                                    '长沙': [111.720664, 27.695864],
                                    '长沙': [112.979353, 28.213478],
                                    '常德': [111.653718, 29.012149],
                                    '郴州': [113.037704, 25.782264],
                                    '衡阳': [112.583819, 26.898164],
                                    '怀化': [109.986959, 27.557483],
                                    '娄底': [111.996396, 27.741073],
                                    '邵阳': [111.461525, 27.236811],
                                    '湘潭': [112.935556, 27.835095],
                                    '湘西土家族苗族自治州': [109.745746, 28.317951],
                                    '益阳': [112.366547, 28.588088],
                                    '永州': [111.614648, 26.435972],
                                    '岳阳': [113.146196, 29.378007],
                                    '张家界': [110.48162, 29.124889],
                                    '株洲': [113.131695, 27.827433],
                                    //江苏
                                    '江苏': [119.368489, 33.013797],
                                    '南京': [118.778074, 32.057236],
                                    '常州': [119.981861, 31.771397],
                                    '淮安': [119.030186, 33.606513],
                                    '连云港': [119.173872, 34.601549],
                                    '南通': [120.873801, 32.014665],
                                    '苏州': [120.619907, 31.317987],
                                    '宿迁': [118.296893, 33.95205],
                                    '泰州': [119.919606, 32.476053],
                                    '无锡': [120.305456, 31.570037],
                                    '徐州': [117.188107, 34.271553],
                                    '盐城': [120.148872, 33.379862],
                                    '扬州': [119.427778, 32.408505],
                                    '镇江': [119.455835, 32.204409],
                                    //江西省
                                    '南昌': [115.676082, 27.757258],
                                    '南昌': [115.893528, 28.689578],
                                    '抚州': [116.360919, 27.954545],
                                    '赣州': [114.935909, 25.845296],
                                    '吉安': [114.992039, 27.113848],
                                    '景德镇': [117.186523, 29.303563],
                                    '九江': [115.999848, 29.71964],
                                    '萍乡': [113.859917, 27.639544],
                                    '上饶': [117.955464, 28.457623],
                                    '新余': [114.947117, 27.822322],
                                    '宜春': [114.400039, 27.81113],
                                    '鹰潭': [117.03545, 28.24131],
                                    ///吉林省

                                    '长春': [125.313642, 43.898338],
                                    '白城': [122.840777, 45.621086],
                                    '白山': [126.435798, 41.945859],
                                    '吉林': [126.564544, 43.871988],
                                    '辽源': [125.133686, 42.923303],
                                    '四平': [124.391382, 43.175525],
                                    '松原': [124.832995, 45.136049],
                                    '通化': [125.94265, 41.736397],
                                    '延边': [129.485902, 42.896414],
                                    //辽宁
                                    '辽宁': [122.753592, 41.6216],
                                    '沈阳': [123.432791, 41.808645],
                                    '鞍山': [123.007763, 41.118744],
                                    '本溪': [123.778062, 41.325838],
                                    '朝阳': [120.446163, 41.571828],
                                    '大连': [121.593478, 38.94871],
                                    '丹东': [124.338543, 40.129023],
                                    '抚顺': [123.92982, 41.877304],
                                    '阜新': [121.660822, 42.01925],
                                    '葫芦岛': [120.860758, 40.74303],
                                    '锦州': [121.147749, 41.130879],
                                    '辽阳': [123.172451, 41.273339],
                                    '盘锦': [122.073228, 41.141248],
                                    '铁岭': [123.85485, 42.299757],
                                    '营口': [122.233391, 40.668651],
                                    //内蒙古自治区
                                    '内蒙古': [114.415868, 43.468238],
                                    '呼和浩特': [111.660351, 40.828319],
                                    '阿拉善盟': [105.695683, 38.843075],
                                    '包头': [109.846239, 40.647119],
                                    '巴彦淖尔': [107.423807, 40.76918],
                                    '赤峰': [118.930761, 42.297112],
                                    '鄂尔多斯': [109.993706, 39.81649],
                                    '呼伦贝尔': [119.760822, 49.201636],
                                    '通辽': [122.260363, 43.633756],
                                    '乌海': [106.831999, 39.683177],
                                    '乌兰察布': [113.112846, 41.022363],
                                    '锡林郭勒盟': [116.02734, 43.939705],
                                    '兴安盟': [122.048167, 46.083757],
                                    //宁夏回族自治区
                                    '宁夏': [106.155481, 37.321323],
                                    '银川': [106.206479, 38.502621],
                                    '固原': [106.285268, 36.021523],
                                    '石嘴山': [106.379337, 39.020223],
                                    '吴忠': [106.208254, 37.993561],
                                    '中卫': [105.196754, 37.521124],
                                    //青海省
                                    '西宁': [101.767921, 36.640739],
                                    '果洛': [100.223723, 34.480485],
                                    '海东': [102.085207, 36.51761],
                                    '海北': [100.879802, 36.960654],
                                    '海南': [100.624066, 36.284364],
                                    '海西': [97.342625, 37.373799],
                                    '黄南': [102.0076, 35.522852],
                                    '玉树': [97.013316, 33.00624],

                                    //山东
                                    '济南': [117.024967, 36.682785],
                                    '滨州': [117.968292, 37.405314],
                                    '东营': [118.583926, 37.487121],
                                    '德州': [116.328161, 37.460826],
                                    '菏泽': [115.46336, 35.26244],
                                    '济宁': [116.600798, 35.402122],
                                    '莱芜': [117.684667, 36.233654],
                                    '聊城': [115.986869, 36.455829],
                                    '临沂': [118.340768, 35.072409],
                                    '青岛': [120.384428, 36.105215],
                                    '日照': [119.50718, 35.420225],
                                    '泰安': [117.089415, 36.188078],
                                    '威海': [122.093958, 37.528787],
                                    '潍坊': [119.142634, 36.716115],
                                    '烟台': [121.309555, 37.536562],
                                    '枣庄': [117.279305, 34.807883],
                                    '淄博': [118.059134, 36.804685],
                                    //山西省
                                    '太原': [112.550864, 37.890277],
                                    '长治': [113.120292, 36.201664],
                                    '大同': [113.290509, 40.113744],
                                    '晋城': [112.867333, 35.499834],
                                    '晋中': [112.738514, 37.693362],
                                    '临汾': [111.538788, 36.099745],
                                    '吕梁': [111.143157, 37.527316],
                                    '朔州': [112.479928, 39.337672],
                                    '忻州': [112.727939, 38.461031],
                                    '阳泉': [113.569238, 37.869529],
                                    '运城': [111.006854, 35.038859],
                                    //陕西
                                    '西安': [108.953098, 34.2778],
                                    '安康': [109.038045, 32.70437],
                                    '宝鸡': [107.170645, 34.364081],
                                    '汉中': [107.045478, 33.081569],
                                    '商洛': [109.934208, 33.873907],
                                    '铜川': [108.968067, 34.908368],
                                    '渭南': [109.483933, 34.502358],
                                    '咸阳': [108.707509, 34.345373],
                                    '延安': [109.50051, 36.60332],
                                    '榆林': [109.745926, 38.279439],
                                    //四川省
                                    '成都': [104.067923, 30.679943],
                                    '阿坝藏族羌族自治州': [102.228565, 31.905763],
                                    '巴中': [106.757916, 31.869189],
                                    '达州': [107.494973, 31.214199],
                                    '德阳': [104.402398, 31.13114],
                                    '甘孜藏族自治州': [101.969232, 30.055144],
                                    '广安': [106.63572, 30.463984],
                                    '广元': [105.819687, 32.44104],
                                    '乐山': [103.760824, 29.600958],
                                    '凉山彝族自治州': [102.259591, 27.892393],
                                    '泸州': [105.44397, 28.89593],
                                    '南充': [106.105554, 30.800965],
                                    '眉山': [103.84143, 30.061115],
                                    '绵阳': [104.705519, 31.504701],
                                    '内江': [105.073056, 29.599462],
                                    '攀枝花': [101.722423, 26.587571],
                                    '遂宁': [105.564888, 30.557491],
                                    '雅安': [103.009356, 29.999716],
                                    '宜宾': [104.633019, 28.769675],
                                    '资阳': [104.63593, 30.132191],
                                    '自贡': [104.776071, 29.359157],
                                    //西藏自治区
                                    '拉萨': [91.111891, 29.662557],
                                    '阿里': [81.107669, 30.404557],
                                    '昌都': [97.185582, 31.140576],
                                    '林芝': [94.349985, 29.666941],
                                    '那曲': [92.067018, 31.48068],
                                    '日喀则': [88.891486, 29.269023],
                                    '山南': [91.750644, 29.229027],
                                    //新疆维吾尔自治区
                                    '乌鲁木齐': [87.564988, 43.84038],
                                    '阿拉尔': [81.291737, 40.61568],
                                    '阿克苏': [80.269846, 41.171731],
                                    '阿勒泰': [88.137915, 47.839744],
                                    '巴音郭楞': [86.121688, 41.771362],
                                    '博尔塔拉': [82.052436, 44.913651],
                                    '昌吉': [87.296038, 44.007058],
                                    '哈密': [93.528355, 42.858596],
                                    '和田': [79.930239, 37.116774],
                                    '喀什': [75.992973, 39.470627],
                                    '克拉玛依': [84.88118, 45.594331],
                                    '克孜勒苏': [76.137564, 39.750346],
                                    '石河子': [86.041865, 44.308259],
                                    '塔城': [82.974881, 46.758684],
                                    '图木舒克': [79.198155, 39.889223],
                                    '吐鲁番': [89.181595, 42.96047],
                                    '五家渠': [87.565449, 44.368899],
                                    '伊犁': [81.297854, 43.922248],

                                    //云南
                                    '昆明': [102.714601, 25.049153],
                                    '保山': [99.177996, 25.120489],
                                    '楚雄': [101.529382, 25.066356],
                                    '大理': [100.223675, 25.5969],
                                    '德宏': [98.589434, 24.44124],
                                    '迪庆': [99.713682, 27.831029],
                                    '红河': [103.384065, 23.367718],
                                    '丽江': [100.229628, 26.875351],
                                    '临沧': [100.092613, 23.887806],
                                    '怒江': [98.859932, 25.860677],
                                    '普洱': [100.980058, 22.788778],
                                    '曲靖': [103.782539, 25.520758],
                                    '昭通': [103.725021, 27.340633],
                                    '文山': [104.089112, 23.401781],
                                    '西双版纳': [100.803038, 22.009433],
                                    '玉溪': [102.545068, 24.370447],
                                    //浙江省
                                    '杭州': [120.219375, 30.259244],
                                    '湖州': [120.137243, 30.877925],
                                    '嘉兴': [120.760428, 30.773992],
                                    '金华': [119.652576, 29.102899],
                                    '丽水': [119.929576, 28.4563],
                                    '宁波': [121.579006, 29.885259],
                                    '衢州': [118.875842, 28.95691],
                                    '绍兴': [120.592467, 30.002365],
                                    '台州': [121.440613, 28.668283],
                                    '温州': [120.690635, 28.002838],
                                    '舟山': [122.169872, 30.03601],

                                    '香港': [114.186124, 22.293586],
                                    '澳门': [113.557519, 22.204118],
                                    '台湾': [120.961454, 23.80406]
                                }
                            },
                            {
                                name: fromcity,
                                type: 'map',
                                mapType: 'china',
                                data: [],
                                markLine: {
                                    smooth: true,
                                    effect: {
                                        show: true,
                                        scaleSize: 1,
                                        period: 30,
                                        color: '#fff',
                                        shadowBlur: 10
                                    },
                                    itemStyle: {
                                        normal: {
                                            borderWidth: 1,
                                            lineStyle: {
                                                type: 'solid',
                                                shadowBlur: 10
                                            }
                                        }
                                    },
                                    data: dataVar
                                },
                                markPoint: {
                                    symbol: 'emptyCircle',
                                    symbolSize: function (v) {
                                        return 10 + v / 10
                                    },
                                    effect: {
                                        show: true,
                                        shadowBlur: 0
                                    },
                                    itemStyle: {
                                        normal: {
                                            label: {show: false}
                                        },
                                        emphasis: {
                                            label: {position: 'top'}
                                        }
                                    },
                                    data: endVar
                                }
                            }
                        ]
                    };
                    myChart.setOption(option);
                }
            });
        }


        /*if (myChart && myChart.dispose) {
         myChart.dispose();
         }
         window.onresize = myChart.resize;
         BMapExt.setOption(option, true)*/
    });
</script>
</body>
</html>

