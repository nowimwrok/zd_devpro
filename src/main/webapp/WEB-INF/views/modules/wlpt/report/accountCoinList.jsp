						...
						$("#btnReset")
								.click(
										function() {
											...
											$("#j_money").val('');
											$("#c_time").val('');
											location.href = "${ctx}/wlpt/consume/useraccount/typeList?models=9";
										});
					});
	function page(n, s) {
		$("#pageNo").val(n);
		...
		$("#searchForm").attr("action",
				"${ctx}/wlpt/consume/useraccount/typeList?models=9");
		$("#searchForm").submit();
		return false;
	}
	$(document)
															...
															function(v, h, f) {
																if (v == "ok") {
																	$(
																			...
																			.attr(
																					"action",
																					"${ctx}/wlpt/consume/useraccount/coinVoExport");
																	$(
																			"#searchForm")
																			.submit();
																}
						...
						$("#btnSubmit")
								.click(
										function() {
											$("#searchForm")
													.attr("action",
															"${ctx}/wlpt/consume/useraccount/coinVo");
											$("#searchForm").submit();
										});
						$("#btnImport")
								.click(
...
<body>
	...
	<form:form id="searchForm" modelAttribute="account"
		action="${ctx}/wlpt/consume/useraccount/coinVo?tabPageId=jerichotabiframe_62"
		...
		<tbody>
			<c:forEach items="${page.list}" var="account" varStatus="step_status">
				<tr>
					...
					<td>
						<fmt:formatNumber value="${account.user.userPurse.availablecoin+account.user.userPurse.freezecoin}" type="Double"
					...
					<td>${account.newforzenonlinecoin }</td>
