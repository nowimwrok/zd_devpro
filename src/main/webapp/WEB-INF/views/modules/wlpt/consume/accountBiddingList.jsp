						...
						$("#btnReset")
								.click(
										function() {
											...
											$("#j_money").val('');
											$("#c_time").val('');
											location.href = "${ctx}/wlpt/consume/useraccount/typeList?models=5";
										});
						$("#btnSubmit")
								.click(
										function() {
											$("#searchForm")
													.attr("action",
															"${ctx}/wlpt/consume/useraccount/typeList?models=5");
											$("#searchForm").submit();
										});
					});
	function page(n, s) {
															...
															function(v, h, f) {
																if (v == "ok") {
																	$(
																			...
																			.attr(
																					"action",
																					"${ctx}/wlpt/consume/useraccount/typeExport?models=5");
																	$(
																			"#searchForm")
																			.submit();
																}
...
<body>
	<ul class="nav nav-tabs">
		<li><a
			href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=3"
			title="查看所有交易">竞标交易流水详情</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="account"
		action="${ctx}/wlpt/consume/useraccount/typeList?models=5&tabPageId=jerichotabiframe_62"
	...
	<table id="contentTable"
		...
		<thead>
			<tr>
				...
				<th colspan="4" class="center ">竞标</th>
			</tr>
			<tr>
				<!-- 账户信息 -->
				<th>登录名</th>
			...
			<c:forEach items="${page.list}" var="account" varStatus="step_status">
				<tr>
					...
					<%--  <td>user. 
							${user.id} </a> </td> --%>
					<td><a
						href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=3&id=${account.user.id}"
