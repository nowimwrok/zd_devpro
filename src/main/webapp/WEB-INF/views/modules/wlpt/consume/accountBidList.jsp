						...
						$("#btnReset")
								.click(
										function() {
											...
											$("#j_money").val('');
											$("#c_time").val('');
											location.href = "${ctx}/wlpt/consume/useraccount/typeList?models=6";
										});
					});
	function page(n, s) {
		$("#pageNo").val(n);
		...
		$("#searchForm").attr("action",
				"${ctx}/wlpt/consume/useraccount/typeList?models=6");
		$("#searchForm").submit();
		return false;
	}
	$(document)
															...
															function(v, h, f) {
																if (v == "ok") {
																	$(
																			...
																			.attr(
																					"action",
																					"${ctx}/wlpt/consume/useraccount/typeExport?models=6");
																	$(
																			"#searchForm")
																			.submit();
																}
...
<body>
	<ul class="nav nav-tabs">
		<li><a
			href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=2"
			alt="查看所有交易">招标交易流水详情</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="account"
		action="${ctx}/wlpt/consume/useraccount/typeList?models=6&tabPageId=jerichotabiframe_62"
			...
			<c:forEach items="${page.list}" var="account" varStatus="step_status">
				<tr>
					...
					<%--  <td>user. 
							${user.id} </a> </td> --%>
					<td><a
						href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=2&id=${account.user.id}"
