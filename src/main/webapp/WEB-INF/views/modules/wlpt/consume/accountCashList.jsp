						...
						$("#btnReset")
								.click(
										function() {
											...
											$("#j_money").val('');
											$("#c_time").val('');
											location.href = "${ctx}/wlpt/consume/useraccount/typeList?models=2";
										});
					});
	function page(n, s) {
		$("#pageNo").val(n);
		...
		$("#searchForm").attr("action",
				"${ctx}/wlpt/consume/useraccount/typeList?models=2");
		$("#searchForm").submit();
		return false;
	}
	$(document)
															...
															function(v, h, f) {
																if (v == "ok") {
																	$(
																			...
																			.attr(
																					"action",
																					"${ctx}/wlpt/consume/useraccount/typeExport?models=2");
																	$(
																			"#searchForm")
																			.submit();
																}
...
<body>
	<ul class="nav nav-tabs">
		<li><a
			href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=4"
			title="查看所有交易">提现交易流水详情</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="account"
		action="${ctx}/wlpt/consume/useraccount/typeList?models=2&tabPageId=jerichotabiframe_62"
	...
	<table id="contentTable"
		...
		<thead>
			<tr>
				...
				<th colspan="4" class="center ">提现</th>
			</tr>
			<tr>
				<!-- 账户信息 -->
				<th>登录名</th>
			...
			<c:forEach items="${page.list}" var="account" varStatus="step_status">
				<tr>
					...
					<%--  <td>user. 
							${user.id} </a> </td> --%>
					<td><a
						href="${ctx}/wlpt/consume/useraccount/detailByType?trademodel=4&id=${account.user.id}"
