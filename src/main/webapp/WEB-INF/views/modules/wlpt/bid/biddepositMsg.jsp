...
<script type="text/javascript">
	$(document).ready(function() {
		$("#btnReset").click(function() {
			...
			$("#j_money").val('');
			$("#c_time").val('');
			location.href = "${ctx}/wlpt/bid/biddeposit/";
		});
	});
	function page(n, s) {
		$("#pageNo").val(n);
															...
															function(v, h, f) {
																if (v == "ok") {
																	$(
																			...
																			.attr(
																					"action",
																					"${ctx}/wlpt/bid/biddeposit/export");
																	$(
																			"#searchForm")
																			.submit();
																}
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/wlpt/bid/biddeposit/">竞标财务列表</a></li> 
		<li><a href="${ctx}/wlpt/bid/biddeposit/detail" alt="查看所有交易">账户竞标详情</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="account"
		action="${ctx}/wlpt/bid/biddeposit/list"
		...
		<tbody>
			<c:forEach items="${page.list}" var="account" varStatus="step_status">
				<tr>
					...
					<td><a
						href="${ctx}/wlpt/bid/biddeposit/detail?id=${account.user.id}"
						target="_self" > ${account.user.loginName} </a></td>
					<td>${account.user.name}</td>
					<td>${account.user.role.name }</td>
					<td>${account.user.phone}</td>
					<!-- 竞标 -->
					<td>${account.jnjbyjAccount }</td>
					<td>${account.sdjbyjAccount }</td>
					<td>${account.jbyjhkAccount }</td>
					<td>${account.sdjbhkAccount }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
