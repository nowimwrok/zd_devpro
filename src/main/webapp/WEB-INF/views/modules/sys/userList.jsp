...
<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post"
    ...
    <ul class="ul-form">
        <li><label>归属公司：</label><sys:treeselect id="company" name="company.id" value="${user.company.id}"
                                                ...
                                                allowClear="true"/></li>
        <li><label>关键字：</label>
				<form:input path="searchinfo" htmlEscape="false" maxlength="50" class="input-medium" placeholder="请输入登录名、手机号码、公司名称"/></li>
        <li><label>认证状态：</label>
            <form:select path="userinfo.status"
                                             class="input-medium">
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" var="user">
        <tr>
            ...
            <td>${user.name}</td>
             <td>${user.userinfo.companyname}</td>
            <td>${user.phone}</td>
            <td>${user.userinfo.status==1?"认证中":user.userinfo.status==2?"已认证":"未认证"}</td>
            <td>${user.userinfo.identitycard }</td>
            <td>${user.email }</td>
