...
<head>
    ...
    <style type="text/css">
        ...
        #closeNextStep, .J-del-card:hover {
            ...
            background: -webkit-gradient(linear, left top, left bottom, from(#FFB91C),
            to(#FFA700));
            background: -moz-linear-gradient(top, #FFB91C, #FFA700);
            filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#FFB91C',
            endColorstr='#FFA700');
...
<body>
<div ng-app="userBankApp" ng-controller="userBankCtrl">
    <div class="row">
        <div class="col-lg-12">
            <div class="panel">
                <div class="form_container">
                    <h4 class="panel-heading ">绑卡信息</h4>
                </div>
                <div class="card_list_wrap mt-10 clearfix">
                    <div class="col-lg-4 col-md-6 col-xs-12 has-js" ng-repeat="x in datas">
                        <div class="card_list clearfix">
                            <div class="clearfix">
                                <div
                                        class="card_id col-sm-7 col-md-7 col-xs-6 text-left clearfix">
                                        <img src="${ctxStatic}/front/img/card_head.png" style="width:50px;margin-right:4px;"/>
                                    <strong ng-cloak style="font-size:16px;">{{x.banktype}}</strong>
                                </div>
                                <div class="card_type col-sm-5 col-md-5 col-xs-6 text-right">
                                    <div class="checkboxes">

                                        <label id="cklabel" class="label_check_y label_check" for="isAccount" ng-if="x.isdefault==1">
                                            <input type="checkbox" id="isAccount"
                                                   style="height: 13px; width: 20px;" name="isAccount"
                                                   ng-click="change(x.id)" checked/>默认
                                        </label>
                                        <label id="cklabelno" class="label_check_n label_check" for="isAccount"
                                               ng-if="x.isdefault==0">
                                            <input type="checkbox" id="isAccountno"
                                                   style="height: 13px; width: 20px;" name="isAccount"
                                                   ng-click="change(x.id)"/>默认
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="card_number" style="background-color:#f00000;" ng-cloak>{{x.bankcard.substr(0,4)+" ****
                                ****
                                "+x.bankcard.substr(x.bankcard.length-4,x.bankcard.length)}}
                            </div>
                            <div class="clearfix" style="margin-top: 25px">
                                <div class="pull-left">
                                    <div class="card_location" ng-cloak>
                                        预留手机号：{{x.bindphone.substr(0,3)+"****"+x.bindphone.substr(7,11)}}
                                    </div>
                                    <div class="card_location" ng-cloak>绑定时间：{{x.createDate.substr(0,10)}}</div>
                                </div>
                                <div class="pull-right col-sm-4" style="padding-top:8px;">
                                    <button class="btn btn-xs btn-bg-red del-card" ng-click="deteleAccount(x)"
                                    >删除
                                    </button>
                                </div>
                            </div>
                    ...
                    <!--删除弹框-->
                    <div id="del-modal" class="modal phone-modal fade in">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">确定要删除此银行卡吗？</div>
                                <div class="modal-footer">
                                    <a href="#" class="btn btn-warning mt-10" data-dismiss="modal">取消</a>
                                    <a href="${wlpt}/wlpt/user/userBank/toDelBankcard?id=" class="btn btn-primary mt-10"
                                       id="ensure_del"
                    ...
                    <div class="card_list_rows">
                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="add_card" style="border: none;"
                                 ng-if="datas==null || datas.length<3 ">
                                <a href="${wlpt}/wlpt/user/userBank/toBankbind"> <i
                                        class="add_icon"> <img
                                        src="${ctxStatic}/images/wlptfront/add.png">
                                </i> <span>添加银行卡</span>
                                </a>
                            </div>
                        </div>
...
<script src="${ctxStatic}/modules/wlpt/front/js/msgbox.js"
        type="text/javascript"></script>
<script src="/static/front/js/form-comment.js"></script>
<script type="text/javascript">
    $("#ensure_del").click(function () {
        window.location.href = "${wlpt}/wlpt/user/userBank/toDelBankcard?id="
            + data.id;
    });
    ...
    app.controller(
        ...
        function ($scope, $http) {
            ...
            $scope.change = function (id) {
                window.location.href = "${wlpt}/wlpt/user/userBank/isDefault?id="
                    + id;
            };
