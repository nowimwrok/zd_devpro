...
public class User extends DataEntity<User> {
	...
	private int intervalday;	// 天数

	private String searchinfo;  //查询条件

	private Userinfo userinfo;  //用户基本信息

	private UserPurse userPurse;  //用户钱包账户

	private UserCompany userCompany;  ///用户企业资料

	private UserBankinfo bankinfo;  ///银行卡信息
	...
	private boolean ischild;    //是否是子账户

	private Role childRole;   ///子账户角色

	private User childUser;    ///子账户
	
	private String memberId;//账户id
	private SysTradeRule rule;//规则类型
	
	private int pointlevle;//扩展 诚信等级
	
	private String isposuser;//是否绑定pos机
	...
	@Length(min=0, max=11, message="手机长度必须为11位字符")
	@NotNull
	...
	@Length(min=0, max=11, message="手机长度11位字符")
	...
	@ExcelField(title="创建时间", align=1, sort=90)
	public Date getCreateDate() {
		return createDate;
	}

	@ExcelField(title="最后登录IP",  align=1, sort=100)
	...
	@ExcelField(title="最后登录日期", align=1, sort=110)
	...
	public String getIsposuser() {
		return isposuser;
	}

	public void setIsposuser(String isposuser) {
		this.isposuser = isposuser;
	}
	
	@ExcelField(title="账户ID", align=2, sort=71)
	...
	public SysTradeRule getRule() {
		return rule;
	}

	public void setRule(SysTradeRule rule) {
		this.rule = rule;
	}
