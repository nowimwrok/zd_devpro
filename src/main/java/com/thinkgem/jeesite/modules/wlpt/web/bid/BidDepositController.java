 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/wlpt/bid/biddeposit")
public class BidDepositController extends BaseController {
	...
	@RequestMapping(value = "export", method=RequestMethod.POST)
	    public String exportFile(Account account, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
			try {
	    		...
	    		new ExportExcel("竞标押金信息数据", BidDeposit.class).setDataList(page.getList()).write(response, fileName).dispose();
	    		return null;
			} catch (Exception e) {
				addMessage(redirectAttributes, "导出用户失败！失败信息："+e.getMessage());
			}
			return "redirect:" + adminPath + "/wlpt/bid/biddeposit/";
	    }
