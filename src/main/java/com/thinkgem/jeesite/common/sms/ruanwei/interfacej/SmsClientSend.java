package com.thinkgem.jeesite.common.sms.ruanwei.interfacej;

import java.net.URLEncoder;

import com.thinkgem.jeesite.common.sms.ruanwei.tool.SmsClientAccessTool;
...
public class SmsClientSend {
	 ...
	 *            ：必填--短信内容
	 * @param needSign
	 *            ：必填--签名 0为短信自带签名 1为需要签名
	 * @param Extends
	 *            ：必填--扩展字段
	 * @return
	 */
	public static String sendSms3(String url,String accountId,
			String accountPwd, String callee, String content,String needSign,
			String Extends) {
		url+="/SMSService.asmx/SendSMS";
		return sendSms3(url, accountId, accountPwd, callee, content,needSign,
				Extends, null,null, null, null, null, null, null, "POST", "UTF-8", "UTF-8");
	}
	 ...
	 * @return
	 */
	private static String sendSms3(String url,String accountId,
			String accountPwd, String callee, String content,String needSign,
			String Extends,String action,
			String sendTime, String checkContent, String taskName,
			String countNumber, String mobileNumber, String telephoneNumber,
			String sendType, String codingType, String backEncodType) {
		...
		try {
			...
			StringBuffer send = new StringBuffer();
			send.append("accountId=").append(
					URLEncoder.encode(accountId, codingType));
			send.append("&accountPwd=").append(
					URLEncoder.encode(accountPwd, codingType));
			send.append("&callee=").append(callee);
			send.append("&content=").append(
					URLEncoder.encode(content, codingType));
			send.append("&needSign=").append(
					URLEncoder.encode(needSign, codingType));
			send.append("&Extends=").append(
					URLEncoder.encode(Extends, codingType));
			if (sendTime != null && !sendTime.equals("")) {
				send.append("&sendTime=").append(
						URLEncoder.encode(sendTime, codingType));
			}
