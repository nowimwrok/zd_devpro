...
public class BindAccount  extends BaseQuartzJobBean{
	
	UserApplicationAccountService userApplicationAccountService = ServiceHelper.getUserApplicationAccountService();
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        BscJob bscJob = new BscJob();
        BaseJoblog baseJoblog = new BaseJoblog();
        Boolean isLog = true;
        try {
            bscJob.setJobcode(Const.Job_Code.BIND_ACCOUNT.name());
            bscJob = jobBiz.findByCondition(bscJob);
            baseJoblog.setJobid(bscJob.getId());
            isLog = "1".equals(bscJob.getIslog());
	...
	private void acquStart(BscJob pdJobLog, BaseJoblog baseJoblog, Boolean isLog) throws Exception {
        ...
        //定时轮询接口绑定memberID到用户
        userApplicationAccountService.bindAccount();
    }
